...
const coreDetails = {
  version: "0.0.1",
  codename: "Midlight Skibidi Sigma ALPHA", // i'll change it i swear
};
...
const Core = {
  ...
  async loadPackage(path) {
    ...
    let id = makeID(5);
    let closeCallback;
    let pid = Core.processList.length + 1;
    let rootFunctions = {
      Lib: {
        html: Html,
        grabConsole: () => {
        ...
        loadLibrary: async function (lib) {
          ...
          return await Core.loadPackage("lib:" + lib);
        },
        loadComponent: async (cmp) => {
          if (cmp.includes(":")) return false;
          knownLibraries.push(cmp);
          return await Core.loadPackage("components:" + cmp);
        },
      },
      Modal: {
        alert: (message) => {
    ...
    if (pkg.type == "process") {
      ...
      //   taskbarIcon.cleanup();
      //   const index = Core.processList.indexOf(appInfo);
      //   if (index > -1) {
      //     Core.processList.splice(index, 1);
      //     console.log(Core.processList);
      //   }
      // });
      ...
      returns = await pkg.exec(rootFunctions, Core);
      appInfo["proc"] = returns;
      Core.processList.push(appInfo);
      console.log(Core.processList);
    }

    if (pkg.type == "ui") {
      let wrapper = Html.qs("body");
...
Core.loadPackage("system:bootloader");
